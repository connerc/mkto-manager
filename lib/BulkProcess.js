...
class BulkProcess {
        ...
        if (LOGGER) {
            ...
            this.LOGGER = tracer.colorConsole({
                ...
                transport: [
                    ...
                    // },
                    (data) => {
                        //  EMit this logging instance so the script consuming BulkProcess can handle the logger printing or writing
    ...
    async run() {
        ...
        //  Set a Listener to emit our own BulkProcess events on BaseAsset find events
        Handler.on('find_request', (reqConfig) => {
            console.log("Handler.on('find_request')")
        ...
        while (!safetyCheckBreaker && cycleIndex <= cycleMaxIteration && (resultCount != 0 || cycleStart)) {
             ...
             */
            const response = await Handler.find(cycleSearchCriteria)
            ...
            if (response.success) {
                ...
                if (response._data) {
                    ...
                    if (!!response.result) {
                        ...
                        //  Emit the success event with this response
                        ...
                        if (!!this.config.successCallback) {
                            if (this.config.awaitSuccess) {
                                ...
                                //  Store the successCallback value into our request tracking
                                cycleResponse.requests[response._data.requestId].successResponse = this.config
                                    .successCallback(response, cycleOffset)
                                    .then(resp => {
                        ...
                        //  Store the results
                        cycleResponse.results = [...cycleResponse.results, ...response.data];
                    }
                }
            }
