...
class MktoRequest {
    ...
    async getAuthToken() {
        ...
        if (this.apiAuthToken) {
            if (this.apiAuthToken._expire < RightNow.getTime()) return true
        }
        ...
        if (authResp.status == 200) {
            ...
            const AfterResponseDate = new Date()

            //  Store our timestamp when this will expire
            if (authResp.data.expires_in) {
                this.apiAuthToken._expire = (AfterResponseDate.getTime() + authResp.data.expires_in)
            }
            else {
                this.apiAuthToken._expire = (AfterResponseDate.getTime() + 3599)
            }
