...
class BaseAsset {
     ...
     * Methods **********
     ********************/
    async buildFormData(fileStream, fileName = false, fileKey = false) {
        ...
        await Promise.all([
            formData.append('name', this.get('name').toString()),
            formData.append('description', this.get('description')),
            formData.append('folder', JSON.stringify(this.get('folder')).trim()),
            formData.append('insertOnly ', "true")
        ])
