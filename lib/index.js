...
module.exports = function ({ mktoBaseUrl, mktoClientId, mktoClientSecret }) {
	...
	/**
	 * Register our Library Services and Values
	 */
	container.register({
		//  Mkto Credentials
	...
	class mktoManager {
		constructor(container) {
			...
			this.loader("assets", path.join(__dirname, "assets", "!(BaseAsset)*.js"));

			//  Load Database Handlers as ScopedContainer instance under the database prop
			//this.loader("database", path.join(__dirname, "database", "*.js"));  //  Coming soon
		}
		...
		loader(collectionName, globPattern) {
            ...
            const collList = collectionName + "List"
